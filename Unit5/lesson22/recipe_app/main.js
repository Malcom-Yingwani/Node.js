...
router.use(methodOverride("_method", { methods: ["POST", "GET"] })); // we place it before app.use("/", router) so that we first override the method before passing the request to the appropriate route
...
router.use(
  expressSession({
    ...
    cookie: {
      maxAge: 4000000, // time in ms
    },
    resave: false, // do not resave session if no changes have occured to the session object
